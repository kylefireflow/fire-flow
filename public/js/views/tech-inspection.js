...
function bindStepEvents(container) {
  window._nextStep = async () => {
    ...
    if (STEPS[currentStep].id === 'checkpoints') {
      ...
      const missingRequired = cps.filter(c => c.required && c.result === null);
      if (missingRequired.length > 0) {
        notify.error(missingRequired.length + ' required checkpoint' + (missingRequired.length > 1 ? 's' : '') + ' not yet marked (marked with *).');
        return;
      }
      const failNoNote = cps.filter(c => c.result === 'fail' && !(c.notes ?? '').trim());
      if (failNoNote.length > 0) {
        notify.error('Add fail notes for ' + failNoNote.length + ' failed item' + (failNoNote.length > 1 ? 's' : '') + ' before continuing.');
        return;
      }
    }
  ...
  window._setCheckpointNotes = (i, val) => {
    ...
    if (row && val.trim()) {
      ...
      const hint = ta ? ta.nextElementSibling : null;
      if (hint) hint.style.display = 'none';
    }
  };
